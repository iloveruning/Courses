window.onload = function () {
    ...
    function succeedFunction(data2) {

        refreshDataTable(data2);

    };


    var refreshDataTable = function (data2) {
        ...
        var table = $('#courselisttable').DataTable({
            ...
            "processing": false,
            "serverSide": true,
            "sAjaxSource": "/getCoursesByPage",//这个是请求的地址
        ...
        /*console.log(table.context[0].aoData[0]._aData.course.id)*/
        ...
        $(".selectpicker").each(function () {
            ...
            for (var i = data2.data.teachers.length - 1; i >= 0; i--) {
                ...
                if (item == table.context[0].aoData.length) {
                    break;
                }

                for (var j = table.context[0].aoData[item]._aData.teachers.length - 1; j >= 0; j--) {

                    if (table.context[0].aoData[item]._aData.teachers[j].id == data2.data.teachers[i].id) {
                        selectedTeachers[count] = table.context[0].aoData[item]._aData.teachers[j].id;
                        count++;
                    }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(".selectpicker").each(function () {
                    ...
                    for (var j = data.data.data[item].teachers.length - 1; j >= 0; j--) {

                        selectedTeachers[count] = data.data.data[item].teachers[j].id;
                        count++;

                    }

                    /*console.log(selectedTeachers.toString())*/
